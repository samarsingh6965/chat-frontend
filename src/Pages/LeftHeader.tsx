...
const LeftHeader: FC<LeftHeaderProps> = () => {
    ...
    return (
        <div className="w-full h-20 bg-sky-100 flex items-center justify-between px-2 border-b">
            <img onClick={handleProfile} src={userDetails.profileImage === null ? (userDetails.gender === 'male' ? maleavatar : userDetails.gender === 'female' ? femaleavatar : otheravatar) : `${process.env.REACT_APP_API_URL}/${userDetails.profileImage.url}`} alt={'profile'} className='w-14 h-14 min-h-14 min-w-14 rounded-full cursor-pointer' />
